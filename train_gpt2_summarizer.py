...
def train(args, model, tokenizer, train_dataset, valid_dataset, ignore_index):
    ...
    wandb.init(project="comp550-gpt2", entity="haooyuee")
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, 
        num_warmup_steps=128,  # 定义预热步骤
        num_training_steps=650  # 定义总训练步骤
    )
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            print("article len -->: {}".format(len(inputs)))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                wandb.log({"lr": scheduler.get_lr()[0], 
                           "loss": (tr_loss - logging_loss)/args.gradient_accumulation_steps, 
            ...
            if (step + 1) % (10*args.gradient_accumulation_steps) == 0:
                ...
                for key, value in results.items():
                    ...
                    wandb.log({'eval_{}'.format(key): value, "global_step": global_step})
    ...
    if global_step:
        ...
        with open(output_eval_file, "a") as f:
            for key in sorted(results.keys()):
                f.write('\n\n')
                f.write("time = %s, %s = %s, step = %s\n" % (datetime.now().strftime("%d/%m/%Y %H:%M:%S"), key, str(results[key]), str(global_step)))
    return results
...
def main():
    ...
    model_file = os.path.join(args.model_dir, 'model_{}_data{}_trained_after_{}_epochs_only_sum_loss_ignr_pad.bin'.format(args.fp16_opt_level,4096,args.num_train_epochs))
    config_file = os.path.join(args.model_dir, 'config_{}_data{}_trained_after_{}_epochs_only_sum_loss_ignr_pad.json'.format(args.fp16_opt_level,4096,args.num_train_epochs))
